...
void String::SplitString(vector<string>& result, string origin, const string& tok)
{
	...
	while ((cutAt = (int)origin.find_first_of(tok)) != origin.npos/*no position : 못 찾았음*/)
	{
		if (cutAt > 0)
...
bool String::Contatin(const string& str, const string& comp)
{
...
bool String::Contatin(const wstring& str, const wstring& comp)
{
	size_t found = str.find(comp);
